...
<template>
  <div
    class="relative bg-white rounded-xl px-4 pt-12 pb-4 shadow-sm border border-gray-100 flex items-start justify-between gap-3"
  >
    <div class="absolute top-0 w-full flex justify-between items-center px-4 left-0 py-3">
      <p class="text-xs font-medium text-gray-500">
        {{ fmtDate(entry.checkIn) }}
      </p>
      <button
    ...
    <div class="text-right shrink-0">
      ...
      <p class="text-xs text-gray-400 mt-0.5">
        {{ fmtMoney(entry.amount / entry.hoursWorked) }}/h
      </p>
    </div>
  </div>
